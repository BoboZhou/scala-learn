...
object ListTest {
  def main(args: Array[String]) {
    ...
    val lt11 = lst0.map(x => x * 2)
    //将lst0中的偶数取出来生成一个新的集合
    val lst2 = lst0.filter(x => x % 2 == 0)
    val ss2 = lst0.filter((x => x % 2 == 0))
    //将lst0排序后生成一个新的集合
    ...
    val m = Map(("a", 1))
    val string = Source.fromFile(new File("C:\\Users\\bobo\\Documents\\Tencent Files\\1120552581\\FileRecv\\yhzs.txt")).mkString
